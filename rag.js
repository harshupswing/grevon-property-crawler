...
function buildMetaChunks(registry){
  ...
  const propList=props.map(function(p){return '- '+p.name+' ('+p.confidence+')';}).join('\n');
  chunks.push({text:registry.domain+' Portfolio\nTotal: '+props.length+'\nEngine: '+(registry.booking_engine||'unknown')+'\n'+propList,metadata:{url:'https://'+registry.domain+'/',property_id:'portfolio',property_name:'Portfolio',page_type:'booking_metadata',confidence:'high'}});
...
async function embed(texts,type){
  ...
  return(await r.json()).data.map(function(d){return d.embedding;});
}
...
async function ingestPages(pages,registry,log){
  log=log||console.log;
  const coll=collectionName(registry.domain),client=getQdrant();
  await ensureCollection(client,coll);
  const chunks=pages.flatMap(function(p){return chunkPage(p);}).concat(buildMetaChunks(registry));
  log('[rag] '+chunks.length+' chunks');
  const embeddings=[];
  for(let i=0;i<chunks.length;i+=96){
    log('[rag] embedding '+(i+1)+'-'+Math.min(i+96,chunks.length));
    const batch=chunks.slice(i,i+96).map(function(c){return c.text;});
    const vecs=await embed(batch,'document');
    embeddings.push(...vecs);
    if(i+96<chunks.length)await new Promise(function(r){setTimeout(r,300);});
  }
  const points=chunks.map(function(c,i){return{id:Math.floor(Math.random()*1e15),vector:embeddings[i],payload:Object.assign({text:c.text},c.metadata)};});
...
async function queryRAG(domain,question,filters,topK){
  filters=filters||{};topK=topK||6;
  const client=getQdrant(),coll=collectionName(domain),vecs=await embed([question],'query');
  const p={vector:vecs[0],limit:topK,with_payload:true};
  const fkeys=Object.keys(filters);
  if(fkeys.length)p.filter={must:fkeys.map(function(k){return{key:k,match:{value:filters[k]}};})};
  return(await client.search(coll,p)).map(function(r){return{score:r.score,text:r.payload.text,url:r.payload.url,property_id:r.payload.property_id,property_name:r.payload.property_name,page_type:r.payload.page_type,confidence:r.payload.confidence};});
}
async function collectionStats(domain){
  try{const i=await getQdrant().getCollection(collectionName(domain));return{exists:true,collection:collectionName(domain),vectors:i.vectors_count};}
  catch(_){return{exists:false,collection:collectionName(domain)};}
}
